...
//Camera position
vec3 p_camera = vec3(32,20,0);

//Camera lookAt
vec3 l_camera = vec3(2,0,-10);

//Light mouvement circle radius
float light_mvnt = 40.0f; // At 30 we get edge artifacts

//Light position
vec3 p_light = vec3(40,20,0);

//Light lookAt
//vec3 l_light = {0,3,-5};
vec3 l_light = vec3(0,3,-10);
...
// Scale and bias transform, moving from unit cube [-1,1] to [0,1]
	scaleBiasMatrix = T(0.5, 0.5, 0.0) * S(0.5, 0.5, 1.0);
	textureMatrix = scaleBiasMatrix * projectionMatrix * modelViewMatrix;
...
// The shader must be loaded before this is called!
	...
	sphereModel = LoadModel("sphere.obj");
}
...
void drawObjects(GLuint shader)
{
	...
	// One torus
	trans = T(0,4,-16) * S(2.0, 2.0, 2.0); // Apply on both
	mv2 = modelViewMatrix * trans; // Apply on both
	tx2 = textureMatrix * trans;
	// Upload both!
	...
	// The other torus
	trans = T(0,4,-16) * Ry(3.14/2) * S(2.0, 2.0, 2.0); // Apply on both
	mv2 = modelViewMatrix * trans;
	tx2 = textureMatrix * trans;
	// Upload both!
	glUniformMatrix4fv(glGetUniformLocation(projTexShaderId, "modelViewMatrix"), 1, GL_TRUE, mv2.m);
	glUniformMatrix4fv(glGetUniformLocation(projTexShaderId, "textureMatrix"), 1, GL_TRUE, tx2.m);
	...
	// The sphere
	trans = T(0,4,-5) * S(5.0, 5.0, 5.0);
	trans = T(0,4,-4) * S(5.0, 5.0, 5.0);
	mv2 = modelViewMatrix * trans; // Apply on both
	tx2 = textureMatrix * trans;
	// Upload both!
	glUniformMatrix4fv(glGetUniformLocation(projTexShaderId, "modelViewMatrix"), 1, GL_TRUE, mv2.m);
...
int main(int argc, char** argv)
{
	...
	glutRepeatingTimer(20); // MicroGlut only
